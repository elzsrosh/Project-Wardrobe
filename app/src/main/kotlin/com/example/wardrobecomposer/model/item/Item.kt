...
data class Item(
    ...
    @Serializable
    enum class Category {
        ВЕРХ, НИЗ, ПЛАТЬЕ, ВЕРХНЯЯ_ОДЕЖДА, ОБУВЬ, АКСЕССУАРЫ
    }

    @Serializable
    enum class Style {
        ПОВСЕДНЕВНЫЙ, ОФИЦИАЛЬНЫЙ, СПОРТИВНЫЙ, ДЕЛОВОЙ, ВЕЧЕРНИЙ, УЛИЧНЫЙ
    }

    @Serializable
    enum class Material {
        ХЛОПОК, ШЕРСТЬ, ШЁЛК, ДЖИНСА, КОЖА, СИНТЕТИКА
    }
    ...
    data class Color(
        ...
        enum class ColorGroup {
            НЕЙТРАЛЬНЫЙ, ТЁПЛЫЙ, ХОЛОДНЫЙ, ЗЕМЛЯНОЙ, ПАСТЕЛЬНЫЙ, ЯРКИЙ,
            КРАСНЫЙ, ОРАНЖЕВЫЙ, ЖЁЛТЫЙ, ЗЕЛЁНЫЙ, СИНИЙ, ФИОЛЕТОВЫЙ,
            РОЗОВЫЙ, КОРИЧНЕВЫЙ, СЕРЫЙ, ЧЁРНЫЙ, БЕЛЫЙ
        }
